...
5. **ปิดไม่ให้ Ghost admin (`/ghost*`) เข้าถึงจากอินเทอร์เน็ตได้เลย** — bind แค่
   ...
   imperva@172.16.57.192` แล้วเปิด `http://localhost:2368/ghost`) กันไม่ให้คนอื่นแย่งสร้าง owner
   account ก่อนคุณ
...
เว็บ**ขึ้นจริงแล้ว**: **http://172.16.57.192:8587** — Docker container รันอยู่บนเซิร์ฟเวอร์ Ubuntu ของคุณ
(`imperva@172.16.57.192`) ทั้ง `/` (หน้าเดิม) และ `/design` (ดีไซน์ใหม่) เข้าถึงได้ทั้งคู่
...
Live at **http://172.16.57.192:8587**, container `ainaidee-app-1`, `restart: unless-stopped`, on
`imperva@172.16.57.192` (Ubuntu 24.04.2, x86_64, 12 GB RAM, Docker 29.3.0 — a shared demo box
...
1. **DNS pointed at `172.16.57.192`** before Caddy/TLS can go on — it requests the certificate on
...
6. **Do you want the design applied to the real home page?** `/design` is still a proposal, now
   confirmed working (including with an ad blocker on) at http://172.16.57.192:8587/design.
