...
ผ่าน `172.16.57.192:8587/blog` ไปพลางๆ ได้ (route เดียวกัน แค่คนละ host)
...
2. **ตั้งค่า DNS**: `blog.ainaidee.com` → `172.16.57.192` (A record) — จำเป็นสำหรับให้ Ghost ตั้ง
...
3. **ยืนยันว่าจะเปิด Ghost ยังไงในระหว่างที่ยังไม่มี Caddy** — เสนอ: `docker compose --profile blog up -d`
   แล้วเข้าผ่าน `172.16.57.192:<port>/ghost` ชั่วคราว (ต้องเพิ่ม port publish ให้ ghost service ก่อน
