...
actually running (2026-07-31) is phase 1 only — plain HTTP on `203.0.113.10:8587`, no Caddy, no
...
flowchart TB
    ...
    subgraph host["Ubuntu host — 203.0.113.10, Docker 29.3.0<br/>shared box, ~11 other containers also running"]
        direction TB
...
`203.0.113.10:8587`) directly — no TLS, no `/og/*.png` → `.jpg` redirect yet.** The routing
...
sequenceDiagram
    ...
    participant Srv as deploy@203.0.113.10
...
sequenceDiagram
    ...
    Dev->>Srv: point A record at 203.0.113.10, set real email in Caddyfile
...
| Links and OG images point at the wrong domain | `SITE_URL` is baked in at build time — rebuild, do not just restart. Currently baked as `http://203.0.113.10:8587`, by design, until phase 2 |
